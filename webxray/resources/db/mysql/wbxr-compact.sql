...
CREATE TABLE IF NOT EXISTS cookie(id INTEGER NOT NULL AUTO_INCREMENT,name MEDIUMTEXT,secure MEDIUMTEXT,path MEDIUMTEXT,domain MEDIUMTEXT,expires MEDIUMTEXT,httponly MEDIUMTEXT,expiry MEDIUMTEXT,value MEDIUMTEXT,captured timestamp,domain_id INTEGER,FOREIGN KEY (domain_id) REFERENCES domain(id),PRIMARY KEY (id));
CREATE TABLE IF NOT EXISTS page_element_junction(page_id INTEGER,element_id INTEGER,FOREIGN KEY (page_id) REFERENCES page(id),FOREIGN KEY (element_id) REFERENCES element(id),UNIQUE KEY (page_id, element_id));
CREATE TABLE IF NOT EXISTS page_cookie_junction(page_id INTEGER,cookie_id INTEGER,FOREIGN KEY (page_id) REFERENCES page(id),FOREIGN KEY (cookie_id) REFERENCES cookie(id),UNIQUE KEY (page_id, cookie_id));
CREATE TABLE IF NOT EXISTS error(id INTEGER NOT NULL AUTO_INCREMENT,uri MEDIUMTEXT NOT NULL,msg MEDIUMTEXT NOT NULL,recorded timestamp NOT NULL,PRIMARY KEY(id));
